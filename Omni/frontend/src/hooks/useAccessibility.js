...
export const useAccessibility = () => {
    ...
    const analyzeScene = useCallback(async (base64Image, query = "Describe my surroundings.") => {
        ...
        try {
            ...
            formData.append('image', finalBlob, 'capture.jpg');
            ...
            if (navigator.geolocation) {
                try {
                    ...
                    formData.append('current_lon', pos.coords.longitude);
                } catch (e) {
                    console.warn("Could not get geolocation for vision context:", e);
                }
            }
            ...
            const response = await fetch(`${API_BASE}/analyze/vision`, {
                method: 'POST',
                body: formData,
            ...
            setSafetyLevel(data.safety_level || 'Safe');
            const hazardInfo = (data.hazard && data.hazard.toLowerCase() !== "none detected") ? `Caution: ${data.hazard}` : "";
            if (hazardInfo && navigator.vibrate) navigator.vibrate(200);
            const feedback = `${data.scene || ''} ${hazardInfo} ${data.guidance || ''}`.trim();
            ...
            if (error.name === 'AbortError') {
                speak("The analysis is taking longer than usual. Please try again.");
            } else {
        ...
        return new Promise((resolve) => {
            navigator.geolocation.getCurrentPosition(async (pos) => {
                try {
                    ...
                    speak(data.instruction || 'Navigation unavailable.');
                    resolve(data);
                } catch (e) {
    ...
    }, [hasStarted, autoGuidance, isLiveStreaming, captureAndAnalyze]);
